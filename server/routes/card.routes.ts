...
router.delete(
  ...
  async (req: ExpressRequest, res: Response) => {
    ...
    if (!deletedCard)
      return res
        .status(404)
        .send({ code: 404, message: "This credit card is not available" });
    if (user)
      user.cards = user.cards.filter(
        (card) => card.toString() !== deletedCard._id.toString()
      );
    await user?.save();
...
router.get("/:cardnumber", async (req, res) => {
  ...
  if (!card)
    return res
      .status(404)
      .send({ code: 404, message: "Card is not available in our system" });
