...
router.get('/get/:id', authMiddleware, async (req: ExpressRequest, res) => {
    ...
    if(id === "all"){
        const allTransfers: TransferResponse[] = await Transfer.find().populate("senderCard senderId receiverCard receiverId")
        const userTransfers = allTransfers.filter(transfer => transfer.senderId._id.toString() === _id || transfer.receiverId._id.toString() === _id)
        const result = userTransfers.map(transfer => {
            if(transfer.senderId.toString() === _id) return {...transfer, fromUser: true}
            return transfer
        })
        return res.status(200).send({code: 200, result})
    }
    const transfer = await Transfer.findOne({_id: id}).populate('senderCard senderId receiverCard receiverId')
    if(!transfer) return res.status(404).send({code: 404, message: "Transfer is not found"})
